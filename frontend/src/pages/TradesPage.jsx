...
export default function TradesPage() {
  ...
  const totalPnl = journals?.reduce((sum, j) => sum + (Number(j.profitLoss) || 0), 0) ?? null
    ...
    <div>
      ...
      <main className="container" style={{ padding: '32px 24px 64px' }}>
          ...
          <div>
            ...
            <p>
              ...
              {totalPnl !== null && journals?.length > 0 && (
                <>
                  {' · Total P&L '}
                  <span className="mono" style={{ color: totalPnl >= 0 ? 'var(--color-profit)' : 'var(--color-loss)' }}>
                    {totalPnl >= 0 ? '+' : ''}
                    {totalPnl.toFixed(2)}
                  </span>
                </>
        ...
        {journals && journals.length === 0 && !error && (
          <div className="card" style={{ padding: '48px 24px', textAlign: 'center', color: 'var(--color-ink-soft)' }}>
        ...
        {journals && journals.length > 0 && journals.map((journal) => <JournalCard key={journal.journalId} journal={journal} />)}
      </main>
    </div>
  )
}
